...
const PhoneInputAz: React.FC<PhoneInputProps> = ({
  countries,
  className,
  placeholder = "+994 (555) 000-0000",
  onChange,
  ...
  const handlePhoneNumberChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (newPhoneNumber.length > 13) return;
    // Başlanğıc kodu
    const initialCode = countryCodes[selectedCountry];

    // Əgər başlanğıc kodu silməyə çalışırsa, icazə vermə
    if (!newPhoneNumber.startsWith(initialCode)) {
      return;
    }

    // Sadəcə rəqəm və + işarəsinə icazə ver
    const allowedChars = /^[+0-9]*$/;
    if (!allowedChars.test(newPhoneNumber)) {
      return;
    }
  ...
  return (
    <div className="relative flex">
      ...
      <input
        ...
        className={`${className} dark:bg-dark-900 h-11 w-full ${
