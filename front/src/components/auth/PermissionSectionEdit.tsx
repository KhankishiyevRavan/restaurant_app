...
const PermissionsSectionEdit: React.FC<PermissionsSectionProps> = ({
  roles,
  permissions = {
    roles: {},
    users: {},
  ...
  const prevPermissionsStateRef = useRef<any>(null);
  ...
  useEffect(() => {
    const initialRolePermissions: Record<
      string,
    ...
    const formattedRolePermissions: Record<
      string,
      { create: boolean; read: boolean; edit: boolean; delete: boolean }
    > = {};

    if (Array.isArray(permissions?.roles)) {
      permissions.roles.forEach((item: any) => {
        formattedRolePermissions[item.role] = item.permissions;
      });
    }
    console.log(permissions.roles);

    console.log(formattedRolePermissions);
    ...
    setPermissionsState((prev) => {
      const newState = {
        ...
        roles: {
          ...
          ...formattedRolePermissions, // Artıq bu obyekt formatındadır
        },
        manage_operday: {
          ...prev.manage_operday,
            ...
            <div>
              ...
              <div className="space-y-2">
                {activeSubItems.map((subItem) => {
                  ...
                  const isRolePerm = isRolePermission(subItem.id);
...
export default PermissionsSectionEdit;
